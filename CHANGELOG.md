...
### Fixed
- **Backup Restore** - Fixed `No session token provided` error during backup restore. The file upload request now correctly includes the authentication header.
...
### Fixed
- **Ingress Security** - Fixed IP validation logic to check the direct socket connection instead of `X-Forwarded-For` header. This ensures requests from the Ingress proxy are correctly authorized while preserving the original client IP for logging.
